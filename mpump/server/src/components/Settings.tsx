/**
 * Settings panel — organized in collapsible accordion categories.
 */
...
function SectionHeader({ title, expanded, onClick, tag }: { title: string; expanded: boolean; onClick: () => void; tag?: string }) {
  return (
    <button className={`settings-section-header ${expanded ? "expanded" : ""}`} onClick={onClick}>
      <span>{tag && <span className="settings-exp-tag">{tag}</span>}{title}</span>
      <span className="settings-arrow">{expanded ? "▼" : "▶"}</span>
    </button>
  );
}
...
export function Settings({ volume, onVolumeChange, onClose, swing, onSwingChange, previewMode, onPreviewModeChange, shareData, cvEnabled, onCVChange, antiClipMode, onAntiClipChange, command, onAbout, onHelp, onTutorial, onExportSession, onImportSession }: Props) {
  ...
  const refreshToggles = () => forceUpdate(n => n + 1);
    ...
    <div className="settings-overlay" onClick={onClose}>
      <div className="settings-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="settings-accordion">
          <SectionHeader title="Audio" expanded={expanded === "audio"} onClick={() => toggle("audio")} />
          {expanded === "audio" && (
            <div className="settings-accordion-body">
              <div className="settings-label">Master Volume</div>
              <div className="settings-vol-row">
                <input type="range" className="settings-slider" min={0} max={1} step={0.01} value={volume}
              ...
              {onSwingChange && swing != null && (<>
                ...
                <div className="settings-vol-row">
                  <input type="range" className="settings-slider" min={0} max={100} step={1}
              ...
              <div className="settings-toggles">
                ...
                {onAntiClipChange && (
                  <select className="synth-preset-select" value={antiClipMode ?? "limiter"} title="Anti-clip prevents digital clipping"
                    onChange={(e) => onAntiClipChange(e.target.value as "off" | "limiter" | "hybrid")} style={{ fontSize: 11 }}>
                ...
                {command && (
                  <button className={`settings-toggle ${getBool("mpump-metronome") ? "on" : ""}`} title="Click track on quarter notes"
                    onClick={() => { const next = !getBool("mpump-metronome"); setBool("mpump-metronome", next); command({ type: "set_metronome", on: next }); window.dispatchEvent(new Event("mpump-settings-changed")); refreshToggles(); }}>
                ...
                {command && (
                  <button className={`settings-toggle ${getBool("mpump-humanize") ? "on" : ""}`} title="Subtle random velocity variation (±15%)"
                    onClick={() => { const next = !getBool("mpump-humanize"); setBool("mpump-humanize", next); command({ type: "set_humanize", on: next }); window.dispatchEvent(new Event("mpump-settings-changed")); refreshToggles(); }}>
        ...
        {/* ── Display ────────────────────────────────────── */}
        <div className="settings-accordion">
          <SectionHeader title="Display" expanded={expanded === "display"} onClick={() => toggle("display")} />
          {expanded === "display" && (
            <div className="settings-accordion-body">
              <div className="settings-label">Theme</div>
              <div className="settings-palettes">
                {PALETTES.map((p) => (
                  <button key={p.id} className={`settings-palette ${palette === p.id ? "active" : ""}`}
                    title={`${p.name} theme`}
                    onClick={() => {
                      setPalette(p.id);
                      applyPalette(p);
              ...
              <div className="settings-toggles" style={{ marginTop: 8 }}>
                ...
                <select className="synth-preset-select" value={getItem("mpump-kaos-wave", "bars-mirror")} title="KAOS pad visualization"
                  onChange={(e) => { setItem("mpump-kaos-wave", e.target.value); refreshToggles(); }} style={{ fontSize: 11 }}>
                ...
                <select className="synth-preset-select" value={getItem("mpump-mix-fx", "both")} title="Visual feedback on MIX"
                  ...
                  <option value="flash">MIX Effect: Flash</option>
                  <option value="both">MIX Effect: Both</option>
                  <option value="off">MIX Effect: Off</option>
                </select>
              </div>
            </div>
          )}
        ...
        {/* ── Controls ───────────────────────────────────── */}
        <div className="settings-accordion">
          <SectionHeader title="Controls" expanded={expanded === "controls"} onClick={() => toggle("controls")} />
          {expanded === "controls" && (
            <div className="settings-accordion-body">
              <div className="settings-toggles">
                <button className={`settings-toggle ${getBool("mpump-key-lock", true) ? "on" : ""}`} title="Keep bass and synth on the same key and octave"
                  onClick={() => { const next = !getBool("mpump-key-lock", true); setBool("mpump-key-lock", next); window.dispatchEvent(new Event("mpump-settings-changed")); refreshToggles(); }}>
                ...
                <button className={`settings-toggle ${getBool("mpump-jam-identity", true) ? "on" : ""}`} title="Show names and colored trails in jam sessions"
                  onClick={() => { const next = !getBool("mpump-jam-identity", true); setBool("mpump-jam-identity", next); window.dispatchEvent(new Event("mpump-settings-changed")); refreshToggles(); }}>
        ...
        {(onExportSession || onImportSession) && (
          <div className="settings-accordion">
            <SectionHeader title="Session" expanded={expanded === "session"} onClick={() => toggle("session")} />
            {expanded === "session" && (
              <div className="settings-accordion-body">
                <div className="settings-toggles">
                  {onExportSession && (
                    <button className="settings-toggle on" onClick={onExportSession} title="Download full session as JSON">
                      <span className="settings-toggle-dot" />Export
                    </button>
                  )}
                  {onImportSession && (
                    <button className="settings-toggle" onClick={() => {
                      ...
                      <span className="settings-toggle-dot" />Import
                    </button>
                  )}
                </div>
              </div>
        ...
        {/* ── Sync ───────────────────────────────────────── */}
        <div className="settings-accordion">
          <SectionHeader title="Sync" expanded={expanded === "sync"} onClick={() => toggle("sync")} />
          {expanded === "sync" && (
            <div className="settings-accordion-body">
              <div className="settings-toggles">
