              ...
              {state.hasKey && keys && (
                <div className="key-octave-row">
                  <label className="info-row half" title="Constrain notes to a musical scale" style={{ display: "flex", alignItems: "center", gap: 4, cursor: "pointer" }}>
                    ...
                    <KaosDropdown className="kaos-dropdown-pat" title="Scale — constrains notes on MIX and editing, and on playback when Scale Snap is on (Settings). Chromatic = all notes allowed." value={scaleLock} onChange={(v: string) => setScaleLock(v)} options={SCALE_NAMES.map(s => ({ label: s.charAt(0).toUpperCase() + s.slice(1), value: s }))} />
                  </label>
